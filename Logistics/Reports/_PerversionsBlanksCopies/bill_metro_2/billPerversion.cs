...
namespace Logistics
{
	public partial class billPerversion : DataDynamics.ActiveReports.ActiveReport3
	{
		...
		private void billPerversion_FetchData(object sender, FetchEventArgs eArgs)
		{
			...
			string _sPartnerOrderNumber = Fields["PartnerOrderNumber"].Value.ToString();
			if (_sPartnerOrderNumber != null && _sPartnerOrderNumber.Length > 0)
			{
				if (_sPartnerOrderNumber.Contains("/")) 
					_sBillNumber = _sPartnerOrderNumber.Substring(_sPartnerOrderNumber.IndexOf("/") + 1);
				else
					_sBillNumber = _sPartnerOrderNumber;
			}
			else
		...
		{
			txtGroupHeaderBillNumber.Text = _sBillNumber.ToString();
		}
