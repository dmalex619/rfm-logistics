...
namespace Logistics
{
	public partial class billPerversion : DataDynamics.ActiveReports.ActiveReport3
	{
		...
		private void billPerversion_FetchData(object sender, FetchEventArgs eArgs)
		{
			...
			_sShopCode = Fields["ShopCode"].Value.ToString();
		...
		private void GroupHeader1_Format(object sender, EventArgs e)
		{
			...
			{
				txtGroupHeaderPayerName.Text = Fields["PayerFullName"].Value.ToString() + "  Договор " + Fields["ContractNumber"].Value.ToString();
			}
			else
			{
				txtGroupHeaderPayerName.Text = Fields["PayerFullName"].Value.ToString();
			}
		}
		...
		private void PageFooter_Format(object sender, EventArgs e)
		{
			...
			txtGroupFooterShopCode.Text = _sShopCode; 
		}
