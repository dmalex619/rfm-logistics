...
namespace Logistics
{
	public partial class billPerversion : DataDynamics.ActiveReports.ActiveReport3
	{
		...
		private void billPerversion_FetchData(object sender, FetchEventArgs eArgs)
		{
			...
			_sContractNumber = Fields["ContractNumber"].Value.ToString();
		...
		{
			if (_sContractNumber != null && _sContractNumber.Length > 0)
				txtGroupHeaderContractNumber.Text = "Договор " + _sContractNumber;
			else
				txtGroupHeaderContractNumber.Text = "";
		}
		...
		private void GroupHeader2_Format(object sender, EventArgs e)
		{
			if (_sPartnerOrderNumber != null && _sPartnerOrderNumber.Length > 0 && 
				...
				txtGroupHeader2Spec2.Value = _sPartnerOrderNumber.Substring(_sPartnerOrderNumber.IndexOf("/") + 1);
			else
				txtGroupHeader2Spec2.Value = "";
		}
